...
char *export_stats_to_json(struct cl_engine *engine, cli_intel_t *intel)
{
    ...
    if (intel->host_info) {
        buf = ensure_bufsize(buf, &bufsz, curused, strlen(intel->host_info) + 30);
        if (!(buf))
            return NULL;
        snprintf(buf + curused, bufsz - curused, "\t\"host_info\": \"%s\",\n", intel->host_info);
        curused = strlen(buf);
    }
    ...
    if (!(buf))
        return NULL;
    snprintf(buf + curused, bufsz - curused, "\t\"samples\": [\n");
    curused = strlen(buf);
