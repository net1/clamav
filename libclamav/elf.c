...
static uint32_t cli_rawaddr32(uint32_t vaddr, struct elf_program_hdr32 *ph, uint16_t phnum, uint8_t conv, uint8_t *err)
{
    ...
    for (i = 0; i < phnum; i++) {
        uint32_t pv = EC32(ph[i].p_vaddr, conv);
        uint32_t pm = EC32(ph[i].p_memsz, conv);
        /* Use subtraction to avoid overflow: pv <= vaddr && (vaddr - pv) < pm */
        if (pv <= vaddr && (vaddr - pv) < pm) {
            found = 1;
            break;
        }
    }
...
static uint64_t cli_rawaddr64(uint64_t vaddr, struct elf_program_hdr64 *ph, uint16_t phnum, uint8_t conv, uint8_t *err)
{
    ...
    for (i = 0; i < phnum; i++) {
        uint64_t pv = EC64(ph[i].p_vaddr, conv);
        uint64_t pm = EC64(ph[i].p_memsz, conv);
        /* Use subtraction to avoid overflow: pv <= vaddr && (vaddr - pv) < pm */
        if (pv <= vaddr && (vaddr - pv) < pm) {
            found = 1;
            break;
