...
static void spam_x86(struct DISASMED *s, char *hr, size_t hrsz)
{
    ...
    n = snprintf(hr, hrsz, "%s", mnemonic[s->real_op]);
    if (n < 0 || (size_t)n >= hrsz) return;
    hr += n;
    hrsz -= n;

    for (i = 0; i < 3 && hrsz > 1; i++) {
        switch (s->args[i].access) {
            ...
            case ACCESS_REL:
                if (s->args[i].arg.rq >= 0)
                    n = snprintf(hr, hrsz, "%s %lx", comma, (long)s->args[i].arg.q);
                else
                    n = snprintf(hr, hrsz, "%s -%x", comma, -(int)s->args[i].arg.rq);
                if (n < 0 || (size_t)n >= hrsz) return;
                hr += n;
                hrsz -= n;
                break;
            case ACCESS_REG:
                n = snprintf(hr, hrsz, "%s %s", comma, x86regs[s->args[i].reg]);
                if (n < 0 || (size_t)n >= hrsz) return;
                hr += n;
                hrsz -= n;
                break;
            case ACCESS_MEM: {
                const char *gotstuff = "";
                n = snprintf(hr, hrsz, "%s %s ptr ", comma, dis_size[s->args[i].size]);
                if (n < 0 || (size_t)n >= hrsz) return;
                hr += n;
                hrsz -= n;
                if (s->segment) {
                    n = snprintf(hr, hrsz, "%s:", x86regs[s->segment]);
                    if (n < 0 || (size_t)n >= hrsz) return;
                    hr += n;
                    hrsz -= n;
                }
                ...
                *hr   = '\0';
                ...
                if (s->args[i].arg.marg.r1 != X86_REG_INVALID) {
                    switch (s->args[i].arg.marg.scale) {
                        case 1:
                            n = snprintf(hr, hrsz, "%s", x86regs[s->args[i].arg.marg.r1]);
                            if (n < 0 || (size_t)n >= hrsz) return;
                            hr += n;
                            hrsz -= n;
                            gotstuff = "+";
                        ...
                        case 0:
                            break;
                        default:
                            n = snprintf(hr, hrsz, "%s*%d", x86regs[s->args[i].arg.marg.r1], s->args[i].arg.marg.scale);
                            if (n < 0 || (size_t)n >= hrsz) return;
                            hr += n;
                            hrsz -= n;
                            gotstuff = "+";
                ...
                if (s->args[i].arg.marg.r2 != X86_REG_INVALID) {
                    n = snprintf(hr, hrsz, "%s%s", gotstuff, x86regs[s->args[i].arg.marg.r2]);
                    if (n < 0 || (size_t)n >= hrsz) return;
                    hr += n;
                    hrsz -= n;
                    gotstuff = "+";
                ...
                if (s->args[i].arg.marg.disp) {
                    if (*gotstuff == '+' && s->args[i].arg.marg.disp < 0)
                        n = snprintf(hr, hrsz, "-%x", -s->args[i].arg.marg.disp);
                    else
                        n = snprintf(hr, hrsz, "%s%x", gotstuff, s->args[i].arg.marg.disp);
                    if (n < 0 || (size_t)n >= hrsz) return;
                    hr += n;
                    hrsz -= n;
                }
